...
if (hasAll) {
  selectedRuntimes = ['claude', 'opencode', 'gemini', 'codex', 'copilot', 'antigravity', 'cursor', 'codebuddy'];
} else if (hasBoth) {
...
Then re-run: npx get-shit-done-dh@latest
`);
    process.exit(1);
  }
}
...
const banner = '\n' +
  ...
  '  development system for Claude Code, OpenCode, Gemini, Codex, Copilot, Antigravity, Cursor, and CodeBuddy by TÂCHES.\n';
...
if (hasHelp) {
  console.log(`  ${yellow}Usage:${reset} npx get-shit-done-dh [options]\n\n  ${yellow}Options:${reset}\n    ${cyan}-g, --global${reset}              Install globally (to config directory)\n    ${cyan}-l, --local${reset}               Install locally (to current directory)\n    ${cyan}--claude${reset}                  Install for Claude Code only\n    ${cyan}--opencode${reset}                Install for OpenCode only\n    ${cyan}--gemini${reset}                  Install for Gemini only\n    ${cyan}--codex${reset}                   Install for Codex only\n    ${cyan}--copilot${reset}                 Install for Copilot only\n    ${cyan}--antigravity${reset}             Install for Antigravity only\n    ${cyan}--cursor${reset}                  Install for Cursor only\n    ${cyan}--codebuddy${reset}               Install for CodeBuddy only\n    ${cyan}--all${reset}                     Install for all runtimes\n    ${cyan}-u, --uninstall${reset}           Uninstall GSD (remove all GSD files)\n    ${cyan}-c, --config-dir <path>${reset}   Specify custom config directory\n    ${cyan}-h, --help${reset}                Show this help message\n    ${cyan}--force-statusline${reset}        Replace existing statusline config\n\n  ${yellow}Examples:${reset}\n    ${dim}# Interactive install (prompts for runtime and location)${reset}\n    npx get-shit-done-dh\n\n    ${dim}# Install for Claude Code globally${reset}\n    npx get-shit-done-dh --claude --global\n\n    ${dim}# Install for Gemini globally${reset}\n    npx get-shit-done-dh --gemini --global\n\n    ${dim}# Install for Codex globally${reset}\n    npx get-shit-done-dh --codex --global\n\n    ${dim}# Install for Copilot globally${reset}\n    npx get-shit-done-dh --copilot --global\n\n    ${dim}# Install for Copilot locally${reset}\n    npx get-shit-done-dh --copilot --local\n\n    ${dim}# Install for Antigravity globally${reset}\n    npx get-shit-done-dh --antigravity --global\n\n    ${dim}# Install for Antigravity locally${reset}\n    npx get-shit-done-dh --antigravity --local\n\n    ${dim}# Install for Cursor globally${reset}\n    npx get-shit-done-dh --cursor --global\n\n    ${dim}# Install for Cursor locally${reset}\n    npx get-shit-done-dh --cursor --local\n\n    ${dim}# Install for CodeBuddy globally${reset}\n    npx get-shit-done-dh --codebuddy --global\n\n    ${dim}# Install for CodeBuddy locally${reset}\n    npx get-shit-done-dh --codebuddy --local\n\n    ${dim}# Install for all runtimes globally${reset}\n    npx get-shit-done-dh --all --global\n\n    ${dim}# Install to custom config directory${reset}\n    npx get-shit-done-dh --codex --global --config-dir ~/.codex-work\n\n    ${dim}# Install to current project only${reset}\n    npx get-shit-done-dh --claude --local\n\n    ${dim}# Uninstall GSD from Cursor globally${reset}\n    npx get-shit-done-dh --cursor --global --uninstall\n\n  ${yellow}Notes:${reset}\n    The --config-dir option is useful when you have multiple configurations.\n    It takes priority over CLAUDE_CONFIG_DIR / GEMINI_CONFIG_DIR / CODEX_HOME / COPILOT_CONFIG_DIR / ANTIGRAVITY_CONFIG_DIR / CURSOR_CONFIG_DIR environment variables.\n`);
  process.exit(0);
}
...
function convertClaudeToCodebuddyContent(content, isGlobal = false) {
  let c = content;
  if (isGlobal) {
    c = c.replace(/\$HOME\/\.claude\//g, '$HOME/.codebuddy/');
    c = c.replace(/~\/\.claude\//g, '~/.codebuddy/');
  } else {
    c = c.replace(/\$HOME\/\.claude\//g, '.codebuddy/');
    c = c.replace(/~\/\.claude\//g, '.codebuddy/');
  }
  c = c.replace(/\.\/\.claude\//g, './.codebuddy/');
  c = c.replace(/\.claude\//g, '.codebuddy/');
...
function convertClaudeCommandToCodebuddySkill(content, skillName, isGlobal = false) {
  const converted = convertClaudeToCodebuddyContent(content, isGlobal);
  const { frontmatter, body } = extractFrontmatterAndBody(converted);
  if (!frontmatter) return converted;
...
function convertClaudeAgentToCodebuddyAgent(content, isGlobal = false) {
  const converted = convertClaudeToCodebuddyContent(content, isGlobal);
  const { frontmatter, body } = extractFrontmatterAndBody(converted);
  if (!frontmatter) return converted;
...
function copyCommandsAsCodebuddySkills(srcDir, skillsDir, prefix, isGlobal = false) {
  ...
  function recurse(currentSrcDir, currentPrefix) {
    ...
    for (const entry of entries) {
      ...
      content = convertClaudeCommandToCodebuddySkill(content, skillName, isGlobal);
      content = processAttribution(content, getCommitAttribution('codebuddy'));
  ...
  for (const entry of entries) {
    ...
    if (entry.isDirectory()) {
      ...
      if (isOpencode) {
        ...
        content = convertClaudeToCodebuddyContent(content, isGlobal);
        content = processAttribution(content, getCommitAttribution(runtime));
        fs.writeFileSync(destPath, content);
      } else {
        fs.writeFileSync(destPath, content);
    ...
    } else if (isCodebuddy && (entry.name.endsWith('.cjs') || entry.name.endsWith('.js'))) {
      let content = fs.readFileSync(srcPath, 'utf8');
      content = convertClaudeToCodebuddyContent(content, isGlobal);
      fs.writeFileSync(destPath, content);
    } else {
...
function uninstall(isGlobal, runtime = 'claude') {
  ...
  if (isOpencode) {
    ...
    // CodeBuddy: remove commands/gsd/ directory and legacy skills/gsd-*/ directories
...
function install(isGlobal, runtime = 'claude') {
  ...
  if (fs.existsSync(agentsSrc)) {
    ...
    for (const entry of agentEntries) {
      ...
      if (!isCopilot && !isAntigravity && !isCodebuddy) {
        ...
        if (isOpencode) {
          ...
          content = convertClaudeAgentToCodebuddyAgent(content, isGlobal);
        }
...
function finishInstall(settingsPath, settings, statuslineCommand, shouldInstallStatusline, runtime = 'claude', isGlobal = true) {
  ...
  if (shouldInstallStatusline && !isOpencode && !isCodex && !isCopilot && !isCursor && !isCodebuddy) {
    settings.statusLine = {
      type: 'command',
  ...
  if (!isCodex && !isCopilot && !isCursor && !isCodebuddy) {
    writeSettings(settingsPath, settings);
  }
  ...
  if (runtime === 'codebuddy') command = '/gsd-new-project';
  console.log(`
...
function promptRuntime(callback) {
  ...
  ${cyan}7${reset}) CodeBuddy    ${dim}(~/.codebuddy)${reset}
...
`);
  ...
  rl.question(`  Choice ${dim}[1]${reset}: `, (answer) => {
    ...
    const choice = answer.trim() || '1';
    if (choice === '9') {
      callback(['claude', 'opencode', 'gemini', 'codex', 'copilot', 'antigravity', 'codebuddy', 'cursor']);
    } else if (choice === '8') {
      callback(['cursor']);
    } else if (choice === '7') {
      callback(['codebuddy']);
...
if (process.env.GSD_TEST_MODE) {
  module.exports = {
    ...
    convertClaudeToCodebuddyContent,
