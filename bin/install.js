...
if (hasAll) {
  selectedRuntimes = ['claude', 'opencode', 'gemini', 'codex', 'copilot', 'antigravity', 'codebuddy'];
} else if (hasBoth) {
...
const banner = '\n' +
  ...
  '  development system for Claude Code, OpenCode, Gemini, Codex, Copilot, Antigravity, and CodeBuddy by TÂCHES.\n';
...
function copyWithPathReplacement(srcDir, destDir, pathPrefix, runtime, isCommand = false, isGlobal = false) {
  ...
  for (const entry of entries) {
    ...
    if (entry.isDirectory()) {
      ...
      // Skip generic replacement for Copilot/Antigravity/CodeBuddy — dedicated converters handle all paths
      let content = fs.readFileSync(srcPath, 'utf8');
...
function finishInstall(settingsPath, settings, statuslineCommand, shouldInstallStatusline, runtime = 'claude', isGlobal = true) {
  ...
  if (shouldInstallStatusline && !isOpencode && !isCodex && !isCopilot && !isCodebuddy) {
    settings.statusLine = {
      type: 'command',
  ...
  if (!isCodex && !isCopilot && !isCodebuddy) {
    writeSettings(settingsPath, settings);
  }
...
function promptRuntime(callback) {
  ...
  ${cyan}7${reset}) CodeBuddy    ${dim}(~/.codebuddy)${reset} - 腾讯云代码助手
  ${cyan}8${reset}) All
`);
  ...
  rl.question(`  Choice ${dim}[1]${reset}: `, (answer) => {
    ...
    const choice = answer.trim() || '1';
    if (choice === '8') {
      callback(['claude', 'opencode', 'gemini', 'codex', 'copilot', 'antigravity', 'codebuddy']);
    } else if (choice === '7') {
      callback(['codebuddy']);
    } else if (choice === '6') {
