<purpose>
将已完成里程碑中累积的阶段目录归档到 `.planning/milestones/v{X.Y}-phases/` 中。识别哪些阶段属于每个已完成的里程碑，显示演练（dry-run）摘要，并在确认后移动目录。
</purpose>
...
对于每个没有 `-phases` 归档的已完成里程碑，读取已归档的 ROADMAP 快照以确定哪些阶段属于它：
...
将阶段目录与里程碑成员身份进行匹配。仅包含仍存在于 `.planning/phases/` 中的目录。
...
AskUserQuestion: "Proceed with archiving?"，选项为："Yes — archive listed phases" | "Cancel"
...
- [ ] 已识别出所有没有现有阶段归档的已完成里程碑
- [ ] 已根据归档的 ROADMAP 快照确定阶段成员身份
- [ ] 已显示演练摘要并获得用户确认
- [ ] 阶段目录已移至 `.planning/milestones/v{X.Y}-phases/`
- [ ] 更改已提交
